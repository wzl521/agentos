...
Agent OS 是**运行和管理 AI Agent 的底座系统**。它装在用户（或企业）自己的机器上，向上为各类 Agent（运维助手、客服助手、HR 助手、销售助手等）提供统一的运行环境，向下接入模型、渠道、工具、记忆、身份和审计基础设施。
...
这件事容易混淆，需要专门讲一下边界。
![](https://static001.infoq.cn/resource/image/8c/63/8cdd7c1bf2a44631751e40f6da8b5063.png)
...
![](https://static001.infoq.cn/resource/image/a3/bc/a3yy4b3037f02ef11db69034389888bc.png)
...
这一章是整份调研论证的重心。这里要把一个判断讲清楚：企业真正的刚需，不是"Agent OS 这个品类"，而**是"一个私有、可控、可审计的 Agent 统一底座"**。这两者听起来像，但锚点完全不同。前者是一个可能演变的概念，后者是一个不会变的需求。
...
![](https://static001.infoq.cn/resource/image/a7/41/a7d9e1df3f5dfb8ba74e4ec4ca8f6241.png)
...
1. **Skill 和 Tool 来源受控，不做无约束的公开市场**。OryxOS 不提供"任何人上传、任何 Agent 随意拉取"的开放 skill 市场。企业内的 Skill 和 Tool 要经过注册、审核、签名、版本管理，来源可追溯。社区的优质 Skill 可以导入，但要经过企业自己的审查关，而不是默认信任。
2. **最小权限，而不是默认全开**。每个 Agent、每个 Tool 拿到的权限是显式授予的最小集合，文件系统、网络、shell 的访问范围默认收紧，按需放开，而不是 OpenClaw 那种默认能力很大、出事才发现的模式。
3. **沙箱隔离是强制的，不是可选的**。Tool 在隔离环境里执行，有明确的资源和能力边界，多租户之间完全隔离，一个租户、一个 Agent 的越界不会波及其他。
4. **凭证不落地，走企业密钥体系。**API key、token、企业系统的凭证不硬编码、不明文存储，对接企业现有的密钥管理（KMS、Vault 等），凭证的使用全程可审计。
5. **prompt injection 和数据外泄要主动防御。**借鉴 Hermes 的做法，记忆写入和工具输入要经过安全扫描，检测注入和外泄模式；对 Agent 可以访问的数据范围和可以外发的目标做约束。
6. **全链路审计是底座能力，不是事后补。**谁、在什么时候、让哪个 Agent、调了什么 Tool、访问了什么数据、产生了什么结果，全程结构化留痕，可以接入企业现有的审计和 SIEM 系统。这正是 OpenClaw 和 Hermes 都缺、而严监管企业必须有的东西。
...
![](https://static001.infoq.cn/resource/image/c3/54/c30255bfd431607d8735015f31c74a54.png)
...
讲分布式之前要先把一个容易混淆的地方说清楚：这里其实有两层完全不同的"分布式"。一层是**底座本身的分布式部署**，也就是同一个 OryxOS 跑多个副本，解决扛量和高可用，这是运维层面的事，6.1 到 6.4 讲的都是它。另一层更远、也更有意思，是**Agent 之间的分布式协作**，多个 Agent 跨节点、跨部门甚至跨组织地互相发现、委托、协同，这里的"节点"是 Agent 而不是服务器。后者是一个真正前沿的方向，放在 6.5 单独讲。
...
今天这些 Agent 是一个个孤岛，各跑各的。而分布式 Agent 协作要解决的，就是让它们能**跨节点地互相发现、把任务可靠地委托给对方、共享必要的上下文、协同完成一件横跨多方的事**。这里的"节点"是 Agent，整个图景更像一个 Agent 版的分布式系统。
